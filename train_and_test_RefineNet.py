...
##########################################################
##########################################################
def train(args, Coarse_Net_model, Refine_Net_model, Rain_drop_model, SS_model):
    ...
    for ep in range(args.epoch):
        ...
        for idx in range(0, batch_idxs):
            ...
            # output = derain_model(Train_rainy_tensor)
            # derain_output = output
            logits_real_list = Adversary(Train_clean_tensor)
            logits_fake_list = Adversary(derain_output.detach())
            loss2 = Adversary_loss(logits_real_list, logits_fake_list)
            loss2.backward()  # free all buffer
            optimizer_A.step()
            '''
            loss2 = 0
            ######################################
